...
@Configuration
@MapperScan(basePackages = "train.common.dao", sqlSessionTemplateRef = "sqlSessionTemplate")
public class BasicDataSourceConfig {
  ...
  public DataSource masterSlaveDataSource(
      @Qualifier(DataSources.MASTER_DB) DataSource masterDB,
      @Qualifier(DataSources.SLAVE_DB) DataSource slaveDB)
      throws SQLException {
    Map<String, DataSource> dbMap =
        Map.of(DataSources.MASTER_DB, masterDB, DataSources.SLAVE_DB, slaveDB);
    ...
    return MasterSlaveDataSourceFactory.createDataSource(
        dbMap, masterSlaveRuleConfiguration, Maps.newHashMap(), new Properties());
  }
  ...
  @Primary
  @Bean
  public SqlSessionFactory sqlSessionFactory(@Qualifier(DataSources.MASTER_DB) DataSource masterDB)
      throws Exception {
